...
// const divisibleByFiveTwoToThePower = (input) => {const numbers = input.map(arr => arr.filter(val => typeOf(val) === 'number' && !(val %5)));
...
// let findMaleAndFemale = (data) => {
// const = humans = data.filter(person => person.gender === 'male' || person.gemder === 'female');
// return humans.ma(person => person.name).join(' and ');
// };
/* ------------------------------------------------------------------------------------------------
CHALLENGE 5
...
let findShortest = (data) => {
  return data.reduce((short, next) => Number(short.height) < Number(next.height) ? short : next).name
};
